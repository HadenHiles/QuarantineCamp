 ...
 */
define('__THEMEDIR__', ossn_route()->themes . 'QuarantineCamp/');
...
function ossn_goblue_theme_init(){
	...
	//custom quarantine camp class
	ossn_new_css('ossn.style', 'css/core/style');
	ossn_load_css('ossn.style');
    ...
    ossn_extend_view('js/opensource.socialnetwork', 'js/quarantinecamp');

	//Sanitizing of latest friends widget #6
	if(ossn_isLoggedin()){
		ossn_add_hook('newsfeed', 'sidebar:right', 'quarantinecamp_latest_members_widget');
		ossn_add_hook('newsfeed', 'sidebar:right', 'quarantinecamp_latest_friends_widget');
	}
}
function quarantinecamp_latest_members_widget($hook, $type, $return){
	$widget_content = ossn_plugin_view('quarantinecamp/members_widget');
...
function quarantinecamp_latest_friends_widget($hook, $type, $return){
	$widget_content = ossn_plugin_view('quarantinecamp/friends_widget');
	$widget = ossn_plugin_view('widget/view', array(
					'title' => ossn_print('quarantinecamp:latest:friends'),
					'contents' => $widget_content
	));
	$return[] = $widget;
	return $return;
