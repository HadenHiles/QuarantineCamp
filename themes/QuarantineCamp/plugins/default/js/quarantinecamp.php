...
$(document).ready(function() {
	...
	$('.sidebar .sub-menu a').each(function(){
				...
				$(this).parent().attr('data-href-quarantinecamp', $href);
				$(this).parent().tooltip();
	});

	$('#discordchatimage').on('mouseover', function() {
	...
	//same as $QuarantineCampSidebarToggle only revert toogle == 0, toogel = 1 (if)
	$QuarantineCampSidebarToggleRotate = function(){
	...
	$QuarantineCampSidebarToggleRotate();

	$QuarantineCampSidebarToggle = function(){
		var $toggle = $('#sidebar-toggle').attr('data-toggle');
		if ($toggle == 0) {
			$('#sidebar-toggle').attr('data-toggle', 1);
			$('[data-toggle="tooltip"]').tooltip('enable');
	...
	$("body").on('click', '.sidebar-menu .menu-content .sub-menu li', function(e){
			e.stopPropagation();
			$data = $(this).attr('data-href-quarantinecamp');
